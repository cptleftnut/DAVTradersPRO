...
// ⚡ Bolt: Cache NumberFormat instances outside the component to avoid recreating them
// or calling toLocaleString() inside high-frequency WebSocket render loops
const priceFormatter = new Intl.NumberFormat(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 });
const amtFormatter = new Intl.NumberFormat(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 });
              ...
              {asks.map((ask, i) => {
                ...
                return (
                  <div key={`ask-${i}`} className="relative grid grid-cols-3 px-2 py-0.5 group hover:bg-gray-800/50">
                     ...
                     <div className="text-gray-300 text-right z-10">{amtFormatter.format(amt)}</div>
