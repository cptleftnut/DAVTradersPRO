...
// ⚡ Bolt: Cache Intl.NumberFormat instances to prevent UI rendering bottlenecks
// during high-frequency WebSocket updates instead of using inline toLocaleString
const priceFormatter = new Intl.NumberFormat(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 });
const amountFormatter = new Intl.NumberFormat(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 });
              ...
              {asks.map((ask, i) => {
                ...
                return (
                  <div key={`ask-${i}`} className="relative grid grid-cols-3 px-2 py-0.5 group hover:bg-gray-800/50">
                     ...
                     <div className="text-red-400 z-10">{priceFormatter.format(price)}</div>
                     <div className="text-gray-300 text-right z-10">{amountFormatter.format(amt)}</div>
              ...
              {bids.map((bid, i) => {
                ...
                return (
                  <div key={`bid-${i}`} className="relative grid grid-cols-3 px-2 py-0.5 group hover:bg-gray-800/50">
                     ...
                     <div className="text-emerald-400 z-10">{priceFormatter.format(price)}</div>
                     <div className="text-gray-300 text-right z-10">{amountFormatter.format(amt)}</div>
                     <div className="text-gray-500 text-right z-10">{totalFormatter.format(total)}</div>
                  </div>
                )
