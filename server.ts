...
// Security sanitization to avoid leaking API keys to the frontend
function sanitizeBotState(state: BotState): Partial<BotState> {
  ...
  try {
    ...
    if (maintenanceEnabled) {
      ...
      res.json({
        ...
        botState: sanitizeBotState(botState),
        closedOrders,
      });
    } else {
